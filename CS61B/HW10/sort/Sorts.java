...
public class Sorts {
   ...
   **/
  protected final static int HEX = 16;
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    ...
    for (int i = 0; i < keys.length; i++) {
      int key = getKey(keys[i], whichDigit);
      out[count[key]] = keys[i];
      count[key]++;
    }
  ...
  private static int getKey(int num, int digit){
    return (num >> (4 * digit)) & 0xF;
  }
  ...
  public static int[] radixSort(int[] keys) {
    // Replace the following line with your solution.
    int out[] = countingSort(keys, 0);
    for (int i = 1; i < 8; i++) {
      out = countingSort(out, i);
    }
    return out;
  }
