...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int w = (horiz - 1) * vert + horiz * (vert - 1);
    int permutation[] = new int[w];
    for (i = 0;i < w; i++) {
      permutation[i] = i;
    }
    ...
    while (w > 0) {
      randIndex = randInt(w);
      tmp = permutation[w-1];
      permutation[--w] = permutation[randIndex];
      permutation[randIndex] = tmp;  
    }

    int x, y, offset = horiz * (vert - 1);

    w = permutation.length;
    for (i = 0; i < w; i++) {
      int wall = permutation[i];
      if(wall < horiz * (vert - 1)){ // horizontal wall
        x = wall % horiz;
        y = wall / horiz;
        int u = y * horiz + x, d = (y + 1) * horiz + x;
        int s1 = s.find(u);
        int s2 = s.find(d);
        ...
        x = (wall - offset) % (horiz - 1);
        y = (wall - offset) / (horiz - 1);
        int l = y * horiz + x, r = y * horiz + x + 1;
        int s1 = s.find(l);
        int s2 = s.find(r);
        if(s1 != s2){
          vWalls[x][y] = false;
