...
public class ListSorts {
  ...
  private static int SORTSIZE = 1000;
  ...
  public static LinkedQueue makeQueueOfQueues(LinkedQueue q) {
    ...
    if(q == null || q.isEmpty()) return queue;

    while(!q.isEmpty()){
      LinkedQueue itemQueue = new LinkedQueue();
      try{
        itemQueue.enqueue(q.dequeue());
        queue.enqueue(itemQueue);
      } catch(QueueEmptyException e){
        System.out.println("Error:  attempt to dequeue from empty queue.");
        e.printStackTrace();
      }
    }
    return queue;
  ...
  public static void partition(LinkedQueue qIn, Comparable pivot, 
    ...
    while(!qIn.isEmpty()){
      try{
        ...
        int res = ((Comparable) item).compareTo((Comparable) pivot);
        if(res < 0){
          qSmall.enqueue(item);
        }else if(res == 0){
  ...
  public static void quickSort(LinkedQueue q) {
    ...
    Object pivot = q.nth(rand.nextInt(q.size())+1);
