...
class Date {
  ...
  public Date(String s) {
    int idx1 = s.indexOf('/'), idx2 = s.lastIndexOf('/');

    int month = Integer.parseInt(s.substring(0, idx1));
    int day = Integer.parseInt(s.substring(idx1+1, idx2)); 
    int year = Integer.parseInt(s.substring(idx2+1, s.length())); 
  ...
  public int difference(Date d) {
    ...
    int days = 0;
    for(int y = old.year; y < nw.year; y++){
      days += isLeapYear(y) ? 366 : 365;
    }
    days += nw.dayInYear() - old.dayInYear();

    if(this.isBefore(d)) days = -days;
    return days;   // replace this line with your solution
  }
