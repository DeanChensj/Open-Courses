...
public class Tree234 extends IntDictionary {
 ...
 /**
  * bubbleUp() is called when insert() met a 3-key-node, i.e node.keys == 3
  * it moves the middle key to the parent and ajust its siblings 
  * and its son
  */
  private void bubbleUp(Tree234Node node){
    ...
    if(p == null){
      ...
      node.parent = root;
      Tree234Node newNode = new Tree234Node(root, node.key3);
    ...
    Tree234Node newNode = new Tree234Node(p, node.key3);
    int x = node.key2;
    // add middle node to its parent
    if(p.keys == 1){
      if(x < p.key1){
        p.key2 = p.key1;
        p.key1 = x;
