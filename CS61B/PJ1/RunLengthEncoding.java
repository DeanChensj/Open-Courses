...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    ...
    this.width = width;
    this.height = height;
    myList = new DList((short)0, (short)0, (short)0, width * height);
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    for(int i = 0; i < runLengths.length; i++){
      myList.insertEnd((short)red[i], (short)green[i], (short)blue[i], runLengths[i]);
    }
  }
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    int oldRun[] = new int[3], newRun[], cnt = 1;
    oldRun[0] = -1; // tricky
    ...
    for(int j = 0; j < height; j++){
      for(int i = 0; i < width; i++){
        newRun = new int[3];
        newRun[0] = image.getRed(i, j);
        newRun[1] = image.getGreen(i, j);
        newRun[2] = image.getBlue(i, j);

        int t;
        for(t = 0; t < 3; t++){
          if(newRun[t] != oldRun[t]) break;
        }
        if(t == 3){
          cnt++;
        }else if(!flag){
          myList.insertEnd((short)oldRun[0], (short)oldRun[1], (short)oldRun[2], cnt);
          cnt = 1;
        }
        oldRun = newRun;
        if(flag) flag = false;
  ...
  public void check() {
    // Your solution here.
    RunIterator i = iterator();
    int oldRun[] = new int[4], newRun[], sum = 0;
    oldRun[1] = -1; // tricky
    while(i.hasNext()){
      newRun = i.next();
      if(newRun[1] == oldRun[1] && newRun[2] == oldRun[2] 
          && newRun[3] == oldRun[3]){
        System.out.println("Two consecutive run. ");
    ...
    if(sum != width * height){
      System.out.println("length does not meet. ");
      System.out.println("sum = " + sum + " length = " + (width * height));
    }
  }
